...
export function HourRow({ hour, items, onCreateTask }: HourRowProps) {
  ...
  const { targetHour, targetMinute, isCollision, isDragging, setTargetPosition, wasJustResizing, wasJustBlockDragging, blockDrag } = useDragState();
  ...
  const handleRowClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      // Ignore clicks that happen right after a resize or drag operation
      if (wasJustResizing() || wasJustBlockDragging()) return;
    ...
    [isCreating, wasJustResizing, wasJustBlockDragging]
  );
